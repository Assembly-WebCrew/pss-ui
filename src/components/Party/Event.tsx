...
class Event extends React.Component<EventProps, EventState> {
  ...
  getLocation(data: any): EventLocation {
    const eventLocation = this.props.locations.find(
  ...
  parseEvent(data: any, party: Party): PartyEvent {
    ...
    return {
      ...
      originalStartTime: elements.originalStartTime.value
        ? new Date(elements.originalStartTime.value).getTime()
        : new Date(elements.startTime.value).getTime(),
      prepStartTime: elements.prepStartTime.value
        ? new Date(elements.prepStartTime.value).getTime()
        : undefined,
      postEndTime: elements.postEndTime.value
        ? new Date(elements.postEndTime.value).getTime()
        : undefined
    };
  }
  ...
  onAddEvent = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const params: { [key: string]: string } = this.props.match.params;
    const partyEvent: PartyEvent = this.parseEvent(
      event.target,
      params.party || ""
    );
      ...
      <Form onSubmit={this.onAddEvent}>
        <Field title="Name">
          <Input
            ...
            defaultValue={this.state.event && this.state.event.name}
            required
          />
        </Field>
        <Field title="Is public?">
          <Toggle
            name="isPublic"
            selected={this.state.event && this.state.event.isPublic}
          />
        </Field>
        <Field title="Location">
          <Select
            ...
            value={this.state.event && this.state.event.location}
          />
        </Field>
        <Field title="Url">
          <Input
            ...
            name="url"
            placeholder="Url"
            defaultValue={(this.state.event && this.state.event.url) || ""}
          />
        </Field>
        <Field title="Media url">
          <Input
            ...
            defaultValue={(this.state.event && this.state.event.mediaUrl) || ""}
          />
        </Field>
        <Field title="Start time">
          <Input
            ...
            defaultValue={
              (this.state.event && this.getDate(this.state.event.startTime)) ||
              ""
            }
          />
        </Field>
        <Field title="Original start time">
          <Input
            ...
            defaultValue={
              this.state.event &&
              this.getDate(this.state.event.originalStartTime)
            }
          />
        </Field>
        <Field title="End time">
          <Input
            ...
            defaultValue={
              this.state.event && this.getDate(this.state.event.endTime)
            }
          />
        </Field>
        <Field title="Prep start time">
          <Input
            ...
            defaultValue={
              this.state.event && this.getDate(this.state.event.prepStartTime)
            }
          />
        </Field>
        <Field title="Post end time">
          <Input
            ...
            defaultValue={
              this.state.event && this.getDate(this.state.event.postEndTime)
            }
          />
        </Field>
